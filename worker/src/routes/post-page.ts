...
export async function handlePostPage(db: D1Database, slug: string): Promise<Response> {
  ...
  const githubIconSvg = `<svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" aria-hidden="true">
  ...
  const codeLinkHtml = row.source_ref
    ...
    : '';
  const codeFooterHtml = row.source_ref
    ? `<footer class="article-footer">
  <a class="code-link" href="${escapeHtml(row.source_ref)}" target="_blank" rel="noopener">
    ${githubIconSvg}
    <span>전체 코드 보기</span>
  </a>
</footer>`
