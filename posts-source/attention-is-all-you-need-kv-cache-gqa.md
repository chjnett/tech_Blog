...
**다음 단계로 해볼 만한 것**:
- 실제 오픈소스 구현(llama.cpp, HuggingFace transformers)의 attention 모듈을 더 깊이 뜯어보며 이 미니 구현체와 실제 프로덕션 코드의 차이(양자화, FlashAttention 커널, 배치 처리 등) 비교
- GQA 그룹 크기(n_rep)를 더 세밀하게 바꿔가며 품질-속도 트레이드오프 곡선 그려보기
- 같은 방식으로 Multi-head Latent Attention(MLA)까지 미니 구현체에 추가해서 3자 비교로 확장
